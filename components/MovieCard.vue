<template>
  <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group flex flex-col h-full">
    <NuxtLink :to="`/movie/${movie.id}`" class="flex-grow flex flex-col">
      <!-- Poster -->
      ...
      <div class="p-4 flex flex-col justify-between flex-grow">
        <div>
          <h3 class="text-lg sm:text-xl font-bold text-gray-900 leading-tight mb-1">
            {{ movie.title }}
          </h3>
        ...
        <p
          class="text-gray-700 text-sm mt-3 line-clamp-3 sm:line-clamp-4 lg:line-clamp-5"
        >
          {{ movie.overview }}
        </p>
      </div>
